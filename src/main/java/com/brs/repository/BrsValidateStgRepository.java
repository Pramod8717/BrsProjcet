...
//	@Query(value = "select * from UAT_XXCD_BRS_VALIDATE_STG_1 where STATUS = ? and BANK_ACCOUNT_NO = ? and unique_reference = ?", nativeQuery = true)
	@Query(value = "select * from UAT_XXCD_BRS_VALIDATE_STG where STATUS = ? and BANK_ACCOUNT_NO = ? and unique_reference = ? LIMIT 1", nativeQuery = true)
	BrsValidateStg getIdFromValidateTbl(String status,String bankAccountNo,String uniqueRef);
	
	@Query(value = "select UNMATCH_REFERENCE from UAT_XXCD_BRS_VALIDATE_STG where STATUS = 'Unreconciled' and BANK_ACCOUNT_NO = ? and unique_reference = ? LIMIT 1", nativeQuery = true)
