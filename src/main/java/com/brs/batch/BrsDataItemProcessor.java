...
public class BrsDataItemProcessor implements ItemProcessor<BrsConsoldtStg, List<BrsConsoldtStg>> {
	...
	public List<BrsConsoldtStg> process(BrsConsoldtStg brsData) {
		...
		log.info("Input to ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
...
//		String uniqueRefNo = util.genUniqueReference(brsData);
		...
		if(brsData.getBankAcNo()!=null&&brsData.getProcessFlag().equals("R")) {
...
//			System.out.println("processedUnmatchNum "+processedUnmatchNum);
		...
		long step002=0,step003=0,step004=0;
		try {
...
//				String unmatchRef = util.getBkcRefId();
				...
				log.info("[CASE 1]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001="+(step001-start1)+" |Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");

				return bookStmtList;
			}
			//long step002 = System.currentTimeMillis();
			/**
...
//				String unmatchRef=util.getBkcRefId();
				...
				long end = System.currentTimeMillis();
				log.info("[CASE 2]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001="+(step001-start1)+" | Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
...
//					String unmatchRef=util.getBkcRefId();
					...
					long end = System.currentTimeMillis();
					log.info("[CASE 3]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001"+(step001-start1)+"  Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
...
//						String unmatchRef=util.getBkcRefId();
						...
						long end = System.currentTimeMillis();
						log.info("[CASE 4]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001"+(step001-start1)+" | Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
...
//					String unmatchRef=util.getBkcRefId();
					...
					long end = System.currentTimeMillis();
					log.info("[CASE 5]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001"+(step001-start1)+" | Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
...
//				bookStmtList = brsDataService.getBrsRecords(uniqueRefNo, bankAcNo);
				...
				bookStmtList =brsDataService.getBrsUnreconRecords(uniqueRefNo, bankAcNo);
...
//				}
				...
				bankStmtList=brsBankStmtService.getBrsUnreconBankStmtData(uniqueRefNo, bankAcNo, loaderProcessId);
...
//				}
				 step003 = System.currentTimeMillis();
				if (brsEbsList != null && !brsEbsList.isEmpty()) {
					log.info("Saving EBS LIST...");
					brsEbsService.saveEbsList(brsEbsList);
				}
				...
				if (brsValidateList != null && !brsValidateList.isEmpty()) {
					log.info("Saving Validate LIST...");
					brsValidateService.saveValidateList(brsValidateList);
				}
				long end = System.currentTimeMillis();
				log.info("[CASE 6]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001 ="+(step001-start1)+"|Time from step001 to step002="+(step002-step001)+"|Time from step002 to step003="+(step003-step002)+" | Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
...
//				String unmatchRef = util.getUnmtchRefId();
				...
				if(bankStmtList!=null&&!bankStmtList.isEmpty()) {
					if(brsEbsList==null) {
						brsEbsList=util.getBrsEBSListForBank(bankStmtList, "Unreconciled", "Amount not matched.", "", unmatchRef);
					}else {
			...
			step004 = System.currentTimeMillis();
			if (brsEbsList != null && !brsEbsList.isEmpty()) {
...
//				bankStmtList.forEach(bs -> bs.setProcessFlag("Y")); // update bank statement process flag
		...
		//long step002 = System.currentTimeMillis();
		long end = System.currentTimeMillis();
		log.info("[CASE 7]Time requried ItemProcessor {}", brsData.getUniqueRef() +" | "+brsData.getBankAcNo()+" | "+brsData.getPaymentMode()+" | "+brsData.getCrAmt()+" | "+brsData.getDrAmt()+" | "+brsData.getVoucherDate()+"| TTP = "+(end-start1)+"| Time from start to step001"+(step001-start1)+"|Time from step001 to step002="+(step002-step001)+"|Time from step002 to step003="+(step003-step002)+"|Time from step003 to step004="+(step004-step003)+" | Thread=[name="+Thread.currentThread().getName()+"|id="+Thread.currentThread().getId()+"]");
