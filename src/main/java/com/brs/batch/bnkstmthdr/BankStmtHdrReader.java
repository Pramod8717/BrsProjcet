...
//	List<String> bnkStmtHdrList=new ArrayList<String>();
...
//	@PostConstruct
	public void init() {
//		bnkStmtHdrList=brsStgService.getBankAccountNumberList();
//		if(AppUtility.bnkStmtHdrList!=null&&!AppUtility.bnkStmtHdrList.isEmpty()) {
			totalBankAccounts=AppUtility.bnkStmtHdrList.size();
			index=0;
//		}
	...
	public void reset() {
		AppUtility.bnkStmtHdrList.clear();
		totalBankAccounts=0;
		index=0;
		counter=0;
	}
	...
	public BrsBankStmtHeaderStg read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException {
		...
		if(AppUtility.bankAccountNo!=null&&!"".equals(AppUtility.bankAccountNo)&&!"null".equals(AppUtility.bankAccountNo)&&
			...
			if(index<totalBankAccounts) {
				String bankAccNum=AppUtility.bnkStmtHdrList.get(index);
